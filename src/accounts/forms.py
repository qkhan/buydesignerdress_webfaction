...
class UserLoginForm(forms.Form):
    ...
    login_or_register = forms.CharField(widget=forms.HiddenInput(), initial='login')

    def clean(self, *args, **kwargs):
        query = self.cleaned_data.get("query")
        password = self.cleaned_data.get("password")
        print ("Clean QUERY 2: ", query)
...
class UserCreationForm(forms.ModelForm):
    ...
    login_or_register = forms.CharField(widget=forms.HiddenInput(), initial='register')
    class Meta:
        model = User
